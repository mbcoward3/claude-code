...
authoring rules. If `plan-ui` is not on `PATH`, run
`python3 <plugin-root>/scripts/plan_ui.py playbook` instead (this skill's
directory sits inside the plugin root).
